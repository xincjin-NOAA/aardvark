...
disk use grows with epochs x val-set size). `bin_name` itself (generated by
`generate_binned_timestamp_list` in `ocelot_vendor/dataset_timeseries.py`,
threaded through as `task["bin_name"]`) is `date=<YYYY-MM-DD>_cycle=<HH>`,
matching the real Parquet partition naming end to end:

```python
import torch
d = torch.load("ocelot_checkpoints/val_outputs/epoch_0/date=2022-02-05_cycle=00.pt")
